...
class GameEngine:
    ...
    def word_control(self):
        word_control = (
            (
                character
                if character not in ascii_lowercase or character in self._right_guesses
                else "_"
            )
            for character in self._game_word
        )

        return "".join(word_control)

    @property
    def ui_word_control(self):
        return colored(
            self.word_control.replace("_", " __ ").upper().rstrip(),
            "cyan",
            attrs=["bold", "italic"],
        )
    ...
    def is_game_won(self):
        required_letters = {
            character for character in self._game_word if character in ascii_lowercase
        }
        return required_letters <= self._right_guesses
    ...
    def history(self):
        ...
        for guess_type, guesses in [
            ("miss", self._misses),
            ("right guess", self._right_guesses),
        ]:
    ...
    def win_game(self):
        print(
            colored(
                f"Congratulations {self._player}! You have won the game",
                "cyan",
                attrs=["bold", "italic"],
            )
        )
    ...
    def lose_game(self):
        print(
            colored(f"Game over! You lose! Sorry {self._player}", "red", attrs=["bold"])
        )
        print(
            "The word is",
            colored(f"{self._game_word.upper()}", "yellow", attrs=["bold", "italic"]),
        )
