...
def test_get_level(monkeypatch, capsys):
    ...
    answers = iter(["2", difficulty])  # option= '2' => single player
    ...
    assert (
        f"Welcome Mayowa, In this level, you will have {lvl.chances} chances to guess"
        in out
    )
    ...
    # test multiplayer
    answers = iter(
        ["1", "1", "Femi", "Mayowa"]  # option= '1' => multi player  # difficulty of 1
    )
    monkeypatch.setattr("builtins.input", lambda _: next(answers))
    ...
    assert (
        f"Welcome Femi, In this level, you will have {lvl.chances} chances to guess"
        in out
    )
